// src/store.js
//
// v2: combines the SQLite migration (trade-off #1), rider auth support
// (trade-off #2), and location tracking (for the native rider app branch)
// into one store, replacing the original JSON-file version.
...
if (riderCount === 0) {
  ...
  const demoHash = bcrypt.hashSync('password123', 10); // demo only - see docs/trade-offs.md
...
const store = {
  ...
  getRiders() {
    return db.prepare('SELECT id, name, phone, lastLat, lastLng, lastLocationAt FROM riders').all();
  },
  ...
  updateRiderLocation(riderId, lat, lng) {
    const rider = db.prepare('SELECT * FROM riders WHERE id = ?').get(Number(riderId));
    if (!rider) return { error: 'rider_not_found' };
    db.prepare('UPDATE riders SET lastLat = ?, lastLng = ?, lastLocationAt = ? WHERE id = ?')
      .run(lat, lng, new Date().toISOString(), riderId);
    return { ok: true };
  },
